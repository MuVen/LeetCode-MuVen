...
public:
    ...
    string tictactoe(vector<vector<int>>& moves) {
        ...
        for(int i = 0; i < nmoves; i++){
            ...
            grid[x][y] = flag == true ? 1 : 2;
        }
        
        int lastmove = (flag == true) ? 1 : 2;
        int lrow = moves[nmoves-1][0], lcol = moves[nmoves-1][1];

        if(grid[lrow][0] == lastmove && grid[lrow][1] == lastmove && grid[lrow][2] == lastmove)
            return lastmove == 1 ? "A" : "B";
        
        if(grid[0][lcol] == lastmove && grid[1][lcol] == lastmove && grid[2][lcol] == lastmove)
            return lastmove == 1 ? "A" : "B";
        
        if(grid[0][0] == lastmove && grid[1][1] == lastmove && grid[2][2] == lastmove)
                return lastmove == 1 ? "A" : "B";
        
        if(grid[0][2] == lastmove && grid[1][1] == lastmove && grid[2][0] == lastmove)
                return lastmove == 1 ? "A" : "B";

        return nmoves == 9 ? "Draw" : "Pending";
    }
};