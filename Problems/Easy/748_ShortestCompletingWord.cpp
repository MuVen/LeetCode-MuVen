...
the letters from the string licensePlate. Such a word is said to complete the 
...
The license plate might have the same letter occurring multiple times.
 For example, given a licensePlate of "PP", the word "pair" does not complete
...
public:
    string shortestCompletingWord(string licensePlate, vector<string>& words) {
        ...
        for(int i = 0; i < words.size(); i++){
            ...
            //lplate characters should present in word characters
            //or less than word characters. else break
            //P(copy);
            ...
            if(j >= 26){
                if(words[i].length() < alength){
                    alength = words[i].length();
                    ans = words[i];
                }
            }
        }
